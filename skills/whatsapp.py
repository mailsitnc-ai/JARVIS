...
SKILL = {
    ...
    "description": "Send a WhatsApp message to a contact, group or number, e.g. 'whatsapp mom saying "
                   ...
                   "'whatsapp +14155551234 saying hi'. Also 'log in to WhatsApp' to link it once. "
                   "Always asks before sending.",
    "triggers": [r"\bwhats?app\b", r"\bwhat'?s\s?app\b"],
    "version": 3,
    "origin": "builtin",
}
# "log in to whatsapp", "connect/set up/link/scan whatsapp", "whatsapp qr" -> the one-time linking flow.
_LOGIN = re.compile(
    r"\b(?:log\s*in|logged\s*in|login|sign\s*in|connect|set\s*up|setup|link|scan|activate|enable|"
    r"authori[sz]e|qr)\b[^.\n]*\bwhats?app\b"
    r"|\bwhats?app\b[^.\n]*\b(?:log\s*in|login|sign\s*in|connect|set\s*up|setup|link|scan|qr\s*code|qr)\b",
    re.IGNORECASE)
...
def run(request, context):
    ...
    # A login/link request (never a real send - it has no message body) -> the one-time QR flow.
    if message is None and _LOGIN.search(request):
        if context.get("dry_run"):
            return "Would open WhatsApp Web in JARVIS's Chrome for a one-time QR scan."
        return _actions(context).whatsapp_login()
